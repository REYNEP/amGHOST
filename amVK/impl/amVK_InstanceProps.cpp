 ...
 */
uint32_t amVK_InstanceProps::ChooseAQueueFamily(VkQueueFlags p_flagBits, amVK_GPU_Index GPU_k) {
...
*/
#include <cstring>
bool amVK_InstanceProps::is_2D_GPU_EXTs_Available(amVK_GPU_Index GPU_k, const char *extName) 
{
...
bool amVK_InstanceProps::is_1D_InstanceEXTs_Available(const char *extName) 
{
     amVK_LOOP_IEXTs(k) {
...
```
 */
void amVK_InstanceProps::addTo_1D_InstanceEXTs_Enabled(const char* extName) {
    // VK_KHR_surface
    if (!amVK_InstanceProps::called_EnumerateInstanceExtensions) {
            amVK_InstanceProps::EnumerateInstanceExtensions();
    }
    
    if (amVK_InstanceProps::is_1D_InstanceEXTs_Available(extName)) {
...
void amVK_InstanceProps::log_1D_InstanceEXTs_Enabled(VkResult ret) {
    if (ret != VK_SUCCESS) {
